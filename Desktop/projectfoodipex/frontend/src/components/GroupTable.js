...
function GroupTable() {
  ...
  const handleProfileOptionClick = (option) => {
    ...
    useEffect(() => {
        ...
        if (loggedInUser) {
            ...
            // Vérifie si l'utilisateur a la permission de voir les groupes
            setCanViewPermission(permissions.some(permission => permission.codename === 'view_permission'));
            // Vérifie si l'utilisateur a la permission d'ajouter des groupes
  ...
  const handleGroupClick = (id) => {
    if (canViewPermission) {
        ...
        setAlert({ message: 'Vous n\'avez pas la permission de voir les permissions de ce groupe.', type: 'error' });
    ...
    {groups
        ...
        .map((group) => (
            <tr key={group.id}>
                ...
                <td onClick={() => handleGroupClick(group.id)} style={{ cursor: 'pointer' }}>
                    {group.name}
                </td>
                <td>
                    <Button
                        style={{ backgroundColor: "white", color: "black", borderColor: "black", opacity: group.name === 'administrateur' ? 0.5 : 1,  }}
                        onClick={(e) => {
                            e.stopPropagation(); // Prevent the row click from triggering
                            handleUpdateClick(group);
                        }}
                        disabled={!canChangeGroup || group.name === 'Administrateurs'}
                    >
                        <FaEdit />
                    </Button>
                    <Button
                        ...
                        disabled={!canDeleteGroup || group.name === 'Administrateurs'}
                        className="ms-2"
                    >
                        <FaTrash />
                    </Button>
